...
#A(198,235) shape=(187,233,3,3), strides=(940,4,940,4)
...
def compute_laplacian(img, mask=None, eps=10**(-7), win_rad=1):
    ...
    if mask is not None:
        ...
        temp = mask.ravel()
        #temp = mask.ravel()[win_inds]
        temp = temp[win_inds]
        win_mask = np.sum(temp, axis=2)
...
def main():
    import argparse
    #------the following arguments are not used anymore--------bymc
    ...
    image = cv2.imread("source.png", cv2.IMREAD_COLOR) / 255.0
    ...
    #from solve_foreground_background import solve_foreground_background    #--method1
    #foreground, _ = solve_foreground_background(image, alpha)              #--method1
    #output = np.concatenate((foreground, alpha[:, :, np.newaxis]), axis=2) #--method1
    output = alpha                                                          #--method2
